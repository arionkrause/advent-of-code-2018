...
//                    if elves_won_with_no_losses(&units) {
//                        eprintln!("power = {:?}", power);
...
//}
//
//fn elves_won_with_no_losses(units: &Vec<Rc<RefCell<Unit>>>) -> bool {
//    !units.iter().any(|u| u.borrow().unit_type == UnitType::Elf
//            && !u.borrow().alive)
//}
//
...
//}
//
//fn attack(unit: &Rc<RefCell<Unit>>, units: &Vec<Rc<RefCell<Unit>>>, power: i16) {
...
//        attacked_unit.borrow_mut().hitpoints -= if unit.borrow().unit_type == UnitType::Elf { power } else { 3 };
