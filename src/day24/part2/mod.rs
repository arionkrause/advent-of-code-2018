...
pub fn solve(input: &str) -> usize {
    ...
    'damage_boost: loop {
        ...
        let mut last_amount_units_remaining = None;

        groups_clone.iter_mut()
                .filter(|group| group.group_type == GroupType::ImmuneSystem)
                .for_each(|group| group.amount_damage += damage_boost);

        loop {
            let targeting = target(&groups_clone);
            attack(&mut groups_clone, &targeting);

            if combat_has_ended(&groups_clone) {
                if immune_system_won(&groups_clone) {
                    return amount_units_remaining(&groups_clone);
                }

                damage_boost += 1;
                continue 'damage_boost;
            } else {
                let amount_units_remaining = amount_units_remaining(&groups_clone);

                if last_amount_units_remaining.is_none() || amount_units_remaining < last_amount_units_remaining.unwrap() {
                    last_amount_units_remaining = Some(amount_units_remaining);
                } else if amount_units_remaining == last_amount_units_remaining.unwrap() {
                    damage_boost += 1;
                    continue 'damage_boost;
                }
            }
...
fn target(groups: &Vec<Group>) -> HashMap<usize, usize> {
    ...
    for index in indices_ordered_by_decreasing_effective_power.into_iter() {
        ...
        for (index_enemy, group_enemy) in groups.iter()
            ...
            if calculated_damage.is_none() || calculated_damage.unwrap() == 0 {
                continue;
            }

            let calculated_damage = calculated_damage.unwrap();
...
fn immune_system_won(groups: &Vec<Group>) -> bool {
